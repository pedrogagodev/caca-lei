    ...
    <Card className="border-2">
      ...
      <div className="grid gap-3 p-4 sm:grid-cols-2">
        ...
        <Card className="flex flex-col px-3 py-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">76% dizem entender o projeto</p>
        </Card>
        <Card className="flex flex-col px-3 py-3">
          <p className="text-xs text-muted-foreground">Tendência</p>
          <div className="mt-2 flex items-end gap-1 h-10">
            {data.distribution.map((item) => (
              <div key={item.label} className="flex h-full w-3 items-end overflow-hidden rounded-full bg-muted">
                <div className="w-full bg-primary" style={{ height: `${item.value}%` }} />
              </div>
            ))}
          </div>
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-[1.2fr_1fr]">
        <Card className="px-3 py-3">
          <p className="text-xs text-muted-foreground">Distribuição de reações</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
            {data.distribution.map((item) => (
              <div key={item.label} className="rounded-lg border bg-muted px-3 py-2">
        ...
        <Card className="px-3 py-3">
          <p className="text-xs text-muted-foreground">Principais temas citados</p>
        ...
        <div>
          ...
          <ul className="mt-2 space-y-2 text-sm">
            {data.insights.map((insight) => (
              <li key={insight} className="rounded-lg border bg-muted px-3 py-2">
                {insight}
              </li>
            ))}
          </ul>
